...
class FuzzyFlowField : PApplet() {
  ...
  override fun draw() {
    ...
    runBlocking {
      repeat(numLines) {
        launch {
          val flowLine = makeFlowFieldLine(angleGrid, Coordinate(random(0f, width.toFloat()).toInt(), random(0f, height.toFloat()).toInt()), random(3f, 10f).toInt(), random(5f, 180f).toInt())
          drawLine(flowLine)
        }
      }
    }
